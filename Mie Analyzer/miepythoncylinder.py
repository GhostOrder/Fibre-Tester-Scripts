...
def mie_S1_S2_cyl(m, x, mu):
    ...
    nangles = len(mu)
    S1 = np.ones(nangles, dtype=np.complex128)*b[0]
    S2 = np.ones(nangles, dtype=np.complex128)*a[0]

    nstop = len(a)
    for k in range(nangles):
        for n in range(1, nstop):
            cosnx = np.real((mu[k]+(1-mu[k]**2)**0.5*1j)**n)
            S1[k] += 2*b[n]*cosnx
            S2[k] += 2*a[n]*cosnx
...
def i_unpolarized_cyl(m,x,mu):
    s1, s2 = mie_S1_S2_cyl(m,x,mu)
    if np.isscalar(mu):
        s1 = s1[0]
        s2 = s2[0]
